...
def deal_data(conn,cursor,list):
    for i in range(0,len(list)-1):
        ...
        if result:
            link_id = result[0]
            sql=("select * from intdata where link_id=%s")
            ...
            if len(result1)==0:
                ...
                sql="update intdata set delay=%s,throughput=%s,packet_loss=%s where link_id=%s"
                cursor.execute(sql,(delay, throughtput, packet_loss,link_id))
                conn.commit()